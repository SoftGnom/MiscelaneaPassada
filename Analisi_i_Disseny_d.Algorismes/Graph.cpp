...
{
	for (CVertex& v : m_Vertices) v.ResetColorRGB();
...
void CGraph::Load(const char* filename)
{
	...
	file.getline(buff, 255);
	if (strncmp("MAXCOLORS ", buff, strlen("MAXCOLORS ")) == 0) {
		m_MaxColors = atoi(buff+ strlen("MAXCOLORS "));
		file.getline(buff, 255);
	}
	else m_MaxColors = -1;
...
void CGraph::Save(const char* filename)
{
	...
	if (m_MaxColors >= 0) file << "MAXCOLORS " << m_MaxColors << endl;
	file << "VERTICES" << endl;
	for ( CVertex &v : m_Vertices) {
...
void CGraph::ColoringToColorRGB()
{
	for (CVertex& v : m_Vertices) {
		if (v.m_Color >= 0) v.m_ColorRGB = (v.m_Color * 9983674) % 0xFFFFFF;
		else v.m_ColorRGB = RGB(255, 0, 0);
	}
}

// ColorsStr =========================================================================

std::string CGraph::ColorsStr()
{
	ostringstream s;
	s << "{";
	for (CVertex& v : m_Vertices) s << v.m_Name << ":" << v.m_Color << " ";
	s << "}";
	return s.str();
}
...
void CGraph::LoadColoring(const char* filename)
{
	ifstream file(filename);
	if (!file.good()) throw MyException("Error obrint graf %s", filename);
	// Llegir la capcelera
	char buff[255];
	file.getline(buff, 255);
	if (strcmp("COLORING 1.0", buff) != 0) throw MyException("Falta la capcalera COLORING 1.0 al fitxer %s", filename);
	// LLegir les distancies
	for (;;) {
		file.getline(buff, 256);
		if (file.eof()) break;
		int d;
		char name[256];
		sscanf_s(buff, "%s %d", name, 256, &d);
		GetVertex(name)->m_Color = d;
	}
	file.close();
	ColoringToColorRGB();
}
...
void CGraph::SaveColoring(const char* filename)
{
	ofstream file(filename);
	if (!file.good()) throw MyException("Error obrint fitxer %s", filename);
	file << "COLORING 1.0" << endl;
	for (CVertex& v : m_Vertices) {
		file << v.m_Name << " " << v.m_Color << endl;
	}
	file.close();
}
