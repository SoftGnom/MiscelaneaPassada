...
public:
	...
	// Atributos coloracio
	int m_Color;
	//Extre:per a utilitzar en diversos algoritmes com: (ColoringBacktracking,SalesmanTrackBacktracking)
	int m_valor;

	//Un node per si fa falta per algun algorisme com el Dijkstra Queque
	CVertex* m_Node;
	//Un areste per si fa falta per algun algorisme com el Dijkstra Queque
	CEdge* m_Aresta;
	...
	void ResetColorRGB() {	m_ColorRGB = RGB(0, 128, 128);	}
	void SetColorRGB(COLORREF color) { m_ColorRGB = color; }
	CVertex(const char* name, double x, double y)
		: m_Name(name)
		, m_ColorRGB(RGB(0, 128, 128))
		, m_Point(x, y)
...
public:
	...
	bool m_valor; //variable per a utilitzar en (Bactraquin normal (com a visitat?), ) 
public:
...
public:
	...
	int m_MaxColors; // Número máximo de colores que se pueden utilziar al colorear
...
public:
	...
	void LoadColoring(const char* filename);
	void SaveColoring(const char* filename);
	void ColoringToColorRGB();
...
CSpanningTree SpanningTreeKruskalLabel(CGraph& g);
CSpanningTree SpanningTreeKruskalTree(CGraph& g);
...
CTrack SalesmanTrackBranchAndBound5(CGraph& g, CVisits& visits);
CTrack SalesmanTrackBranchAndBound6(CGraph& g, CVisits& visits);
CTrack SalesmanTrackBranchAndBound7(CGraph& g, CVisits& visits);
